...
export function categoryOptions(categories:Category[],types?:Category['type'][],parentsOnly=false){return categoryTree(categories,types).filter(row=>!parentsOnly||row.depth===0).map(({category,depth})=><option key={category.id} value={category.id}>{depth?`\u2003↳ ${category.name}`:`${emojiOrFallback(category.icon)} ${category.name}`}</option>)}
