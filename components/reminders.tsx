    ...
    async function run() {
      {
        ...
        for (const kind of fire) {
          if (kind === 'balance') {
            const title = 'Waktunya update saldo 💰', body = `Cocokkan saldo ${data.wallets.filter(w => !w.isArchived).length} dompetmu dan catat transaksi yang terlewat hari ini.`;
...
export function ReminderSettings() {
  ...
  return <div className="panel reminder-settings">
    ...
    {permission !== 'granted' && <div className="notice">{permission === 'unsupported' ? 'Browser ini belum mendukung notifikasi. Pengingat tetap muncul di dalam aplikasi.' : permission === 'denied' ? 'Notifikasi diblokir. Izinkan lewat pengaturan situs di browser agar pengingat muncul di HP.' : <>Izinkan notifikasi agar pengingat muncul di HP. <Button type="button" className="small" onClick={() => void allow()}>Izinkan notifikasi</Button></>}</div>}

    <label className="switch-row"><input type="checkbox" checked={draft.balanceEnabled} onChange={e => set({ balanceEnabled: e.target.checked })}/><span><strong>Ingatkan update saldo</strong><small>Untuk mencocokkan saldo dan mencatat transaksi yang terlewat.</small></span></label>
    ...
    <div className="toolbar-row"><Button type="button" disabled={!dirty} onClick={() => void save()}>Simpan pengingat</Button>{permission === 'granted' && <Button type="button" variant="secondary" onClick={() => void showSystemNotification('Contoh pengingat 💰', 'Waktunya update saldo dompetmu.', '/?view=wallets', 'test')}>Kirim contoh</Button>}</div>
