 ...
 <div className={`set-layout ${active?'has-page':''}`}>
  ...
  {page&&<section className="set-page" key={page.key} aria-labelledby="set-page-title">
   ...
   {page.key==='data'&&<div className="set-form">
    ...
    <div className="set-card"><h3>Sinkronisasi hemat kuota</h3><p>Data tersimpan di perangkat ini. Saat aplikasi dibuka, hanya data yang berubah yang diunduh, jadi kuota baca Firestore tetap kecil. Jika data di perangkat ini terasa tidak cocok dengan perangkat lain, muat ulang semuanya sekali dari cloud.</p><div className="settings-actions start"><Button variant="secondary" onClick={()=>{if(!user)return;resyncAll(user.uid);notify('Semua data sedang dimuat ulang dari cloud.')}}><RefreshCw size={16}/> Sinkronkan ulang</Button></div></div>
