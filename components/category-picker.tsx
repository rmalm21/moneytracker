...
export function CategoryPicker({type,categoryId,subcategoryId,onChange,required=false}:{type:'expense'|'income';categoryId:string;subcategoryId:string;onChange:(value:Choice)=>void;required?:boolean}){
 ...
 return <span className="datetime-control">
  <button type="button" className={`input datetime-trigger ${invalid?'is-invalid':''}`} onClick={show} aria-haspopup="dialog">
   <span className={selectedParent?'':'muted'}>{selectedParent?<>{emojiOrFallback(selectedParent.icon)} {selectedParent.name}{selectedChild&&<> › {emojiOrFallback(selectedChild.icon)} {selectedChild.name}</>}</>:'Pilih kategori'}</span><Tags size={17} aria-hidden="true"/>
  </button>
  ...
  <Dialog open={open} onOpenChange={setOpen}><DialogContent title="Pilih Kategori" className="mobile-sheet category-picker-dialog">
   ...
   :viewing?<>
     <button type="button" className="picker-back" onClick={()=>setParentView(null)}><ChevronLeft size={18}/> {emojiOrFallback(viewing.icon)} {viewing.name}</button>
     <div className="picker-list">{item(viewing,<small>Tanpa subkategori</small>,()=>pick(viewing))}{childrenOf(viewing.id).map(c=>item(c))}</div>
   </>:<>
     {frequent.length>0&&<><h5 className="picker-heading-label">Sering dipakai</h5><div className="picker-chips">{frequent.map(c=><button type="button" key={c.id} className="quick-action" onClick={()=>pick(c)}><span aria-hidden="true">{emojiOrFallback(c.icon)}</span><span>{c.name}</span></button>)}</div></>}
