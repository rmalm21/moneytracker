...
export function TransactionInbox({openTx,notify}:{openTx:(preset?:Partial<LedgerTx>)=>void;notify:(message:string)=>void}){
 ...
 return <><div className="page-heading"><div><h1>Perlu Dikonfirmasi</h1><p>Jadwal rutin di sini belum mengubah saldo. Tinjau sebelum mencatatnya.</p></div></div><div className="panel"><strong>{pending.length} transaksi · {rupiah(pending.reduce((n,item)=>n+item.amount,0))}</strong><small style={{display:'block',marginTop:6}}>Konfirmasi membuat satu transaksi aktual dan menghapus komitmen yang menunggu.</small></div><div className="panel flush" style={{marginTop:17}}>{pending.length?pending.map(item=><div className="data-row" key={item.id}><div className="data-row-main"><strong>{item.name}</strong><small>{item.plannedDate} · Menunggu Konfirmasi · {data.wallets.find(w=>w.id===item.walletId)?.name||'Dompet'}</small></div><div className="data-row-side"><strong>{rupiah(item.amount)}</strong><div className="toolbar-row"><Button className="small" onClick={()=>openTx({type:item.type,amount:item.amount,date:item.plannedDate,walletId:item.walletId,destinationWalletId:item.destinationWalletId,categoryId:item.categoryId,description:item.name,draftId:item.id,recurringTransactionId:item.recurringId})}>Konfirmasi / Edit</Button><Confirm title="Lewati transaksi ini?" description="Transaksi ini tidak akan dicatat ke saldo. Jadwal rutin berikutnya tetap ada." onConfirm={()=>skip(item.id)}><button className="link-button">Lewati</button></Confirm></div></div></div>):<Empty message="Tidak ada transaksi yang menunggu konfirmasi."/>}</div>{error&&<p role="alert" className="form-error">{error}</p>}</>
}
