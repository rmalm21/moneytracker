...
import { emojiKeys } from '@/lib/emoji-set';
...
export const hasEmojiImage = (emoji: string) => emojiKeys.has(emojiKey(emoji));

/** One emoji drawn with the bundled Fluent Emoji artwork, so it looks the same on Android, Windows and iPhone. */
export function Emoji({ e, className = '' }: { e: string; className?: string }) {
  const key = emojiKey(e);
  if (!emojiKeys.has(key)) return <span className={className}>{e}</span>;
...
/** Text with any emoji inside swapped for the Fluent Emoji artwork. */
export function EmojiText({ text }: { text?: string | null }): ReactNode {
  ...
  graphemes(text).forEach((g, i) => { if (pictographic.test(g) && hasEmojiImage(g)) { if (plain) parts.push(plain); plain = ''; parts.push(<Emoji key={i} e={g}/>); } else plain += g; });
  if (plain) parts.push(plain);
  return <Fragment>{parts}</Fragment>;
}

/** Props for a `data-avatar` row icon: the emoji as text, plus its Fluent Emoji artwork when available. */
export function emojiAvatar(emoji: string): { 'data-avatar': string; style?: CSSProperties } {
  const key = emojiKey(emoji);
  return emojiKeys.has(key) ? { 'data-avatar': emoji, style: { '--avatar-img': `url(/emoji/${key}.webp)` } as CSSProperties } : { 'data-avatar': emoji };
}
