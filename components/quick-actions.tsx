...
export function QuickActions({ openTx, navigate, notify }: { openTx: (preset?: Partial<LedgerTx>) => void; navigate: (view: string) => void; notify: (message: string) => void }) {
  ...
  return <>
    ...
    <div className="quick-row">{visible.map(action => { const Icon = action.icon && action.icon !== 'default' ? customIcons[action.icon] : icons[action.kind]; return <button key={action.id} className="quick-action" onClick={() => launch(action)}><Icon size={16}/><span>{action.label}</span>{action.amount ? <small>{rupiah(action.amount)}</small> : null}</button>; })}{visible.length === 0 && <p className="muted">Belum ada tombol. Ketuk “Atur tombol” untuk menambahkannya.</p>}</div>
    <Dialog open={open} onOpenChange={close}><DialogContent title="Atur Catat cepat" className="quick-dialog">
      {editing === null ? <>
        ...
        <Field label="Ikon tombol"><Select value={draft.icon || 'default'} onChange={event => setDraft(previous => ({ ...previous, icon: event.target.value as QuickAction['icon'] }))}><option value="default">Ikon bawaan</option><option value="coffee">Kopi</option><option value="food">Makanan</option><option value="shopping">Belanja</option><option value="transport">Transportasi</option><option value="bill">Tagihan</option><option value="wallet">Dompet</option><option value="star">Bintang</option></Select></Field>
        ...
        {error && <p className="form-error" role="alert">{error}</p>}
        <div className="modal-actions"><Button type="button" variant="secondary" onClick={() => { setEditing(null); setError(''); }}>Kembali</Button><Button type="submit">Selesai</Button></div>
      </form>}
