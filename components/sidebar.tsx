...
export function Sidebar({ items, view, onNavigate }: { items: { key: string; label: string; icon: LucideIcon; section?: string }[]; view: string; onNavigate: (key: string) => void }) {
  ...
  return <aside className={`sidebar ${collapsed ? 'is-collapsed' : ''}`} aria-label="Menu utama">
    ...
    <div className="sidebar-scroll"><p className="nav-label">MENU UTAMA</p>{items.map((item) => { const active = menuKeyOf(view) === item.key; return <div key={item.key}>{item.section&&<p className="nav-label nav-divider">{item.section}</p>}<button type="button" className={`nav-item ${active ? 'active' : ''}`} aria-label={item.label} aria-current={active ? 'page' : undefined} title={collapsed ? item.label : undefined} onClick={() => onNavigate(item.key)}><item.icon size={18}/><span className="nav-text">{item.label}</span></button></div>; })}</div>
