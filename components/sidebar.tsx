...
export function Sidebar({ items, view, onNavigate }: { items: { key: string; label: string; icon: LucideIcon }[]; view: string; onNavigate: (key: string) => void }) {
  ...
  return <aside className={`sidebar ${collapsed ? 'is-collapsed' : ''}`} aria-label="Menu utama">
    <div className="sidebar-header"><div className="brand"><span className="brand-mark"><Sparkles size={19}/></span><strong>dompet ajaib<span className="brand-dot">.</span></strong></div><button type="button" className="sidebar-toggle" onClick={toggle} aria-label={collapsed ? 'Lebarkan menu' : 'Ciutkan menu'} aria-expanded={!collapsed} title={collapsed ? 'Lebarkan menu' : 'Ciutkan menu'}>{collapsed ? <PanelLeftOpen size={18}/> : <PanelLeftClose size={18}/>}</button></div>
