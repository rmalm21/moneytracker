...
export async function settleBudgets(uid:string,budgets:Budget[],salaryDay:number,now=new Date()) {
  for(const budget of budgets.filter(b=>b.rolloverEnabled)) {
    ...
    while(cursor<current.start&&steps++<(budget.cycleType==='weekly'?160:36)){const next=budgetWindow(budget,new Date(cursor+'T12:00:00'),salaryDay).end;if(next<=cursor)throw Error('Periode sisa anggaran tidak dapat dihitung.');const snap=await getDocs(query(coll(uid,'transactions'),where('date','>=',cursor),where('date','<',next)));const tx=snap.docs.map(x=>hydrate<LedgerTx>(x));const cats=await getDocs(coll(uid,'categories'));carry+=budget.amount-budgetSpent(budget,tx,cats.docs.map(x=>hydrate<Category>(x)));cursor=next;}
