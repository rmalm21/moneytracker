...
export async function initProfile(user:User) {
  ...
  if (!snapshot.exists()) await setDoc(r,{uid:user.uid,email:user.email||'',username:(user.email||'').split('@')[0],displayName:user.displayName||'',currency:'IDR',salaryCycleStartDay:24,monthlySalary:0,onboardingDone:false,hideReserved:true,theme:'light',dashboardWidgets:[],createdAt:serverTimestamp(),updatedAt:serverTimestamp(),lastLoginAt:serverTimestamp()});
  else await updateDoc(r,{lastLoginAt:serverTimestamp()});
}
...
export async function saveProfile(uid:string,changes:Partial<Profile>) { await updateDoc(userRef(uid),{...changes,updatedAt:serverTimestamp()}); }
export async function saveWallet(uid:string,input:Partial<Wallet>&{name:string;openingBalance:number},id?:string) {
  if(!Number.isSafeInteger(input.openingBalance)) throw Error('Saldo awal harus bilangan Rupiah yang valid.');
...
export async function upsertTransaction(uid:string,input:LedgerTx,editId?:string) {
  ...
  await runTransaction(database(),async trx=>{
...
export async function deleteTransaction(uid:string,id:string) {
  const r=ref(uid,'transactions',id);let removedDate='';await runTransaction(database(),async trx=>{const snap=await trx.get(r);if(!snap.exists())return;const old=hydrate<LedgerTx>(snap);removedDate=old.date;
    if(['claim_advance','receivable_issue','borrowing'].includes(old.type))throw Error('Transaksi pembuka terkait catatan lain. Hapus atau koreksi catatan induknya terlebih dahulu.');
    ...
    if(side&&rr&&sideSnap?.exists()){const obj=sideSnap.data();const key=side.kind==='claims'||side.kind==='receivables'?'remainingAmount':side.kind==='debts'?'outstandingAmount':'currentAmount';const next=Number(obj[key])+side.delta;trx.update(rr,{[key]:next,...(side.kind==='claims'?{status:next===0?'paid':'waiting'}:{}),...(side.kind==='debts'?{status:next===0?'paid':'open'}:{}),...(side.kind==='receivables'?{status:next===0?'paid':next<Number(obj.originalAmount)?'partial':'open'}:{}),updatedAt:serverTimestamp()});}
...
export async function editOpeningRecord(uid:string,kind:'claims'|'receivables'|'debts',id:string,changes:Record<string,unknown>,newAmount:number){
  ...
  await runTransaction(database(),async trx=>{const rr=ref(uid,kind,id),snapshot=await trx.get(rr);if(!snapshot.exists())throw Error('Catatan tidak ditemukan.');const old=snapshot.data();const txSnap=first?await trx.get(first.ref):null;
...
export async function deleteOpeningRecord(uid:string,kind:'claims'|'receivables'|'debts',id:string){
  ...
  await runTransaction(database(),async trx=>{const rr=ref(uid,kind,id),snap=await trx.get(rr);if(!snap.exists())return;const row=snap.data(),initial=Number(kind==='claims'?row.amount:row.originalAmount),remaining=Number(kind==='debts'?row.outstandingAmount:row.remainingAmount);if(remaining!==initial)throw Error('Catatan yang sudah memiliki pembayaran tidak dapat dihapus. Koreksi transaksi pembayarannya terlebih dahulu.');const t=opening?await trx.get(opening.ref):null;if(opening&&!t?.exists())throw Error('Transaksi pembuka tidak tersedia.');if(t?.exists()){const tx=hydrate<LedgerTx>(t);for(const [wallet,delta] of Object.entries(effects(tx)))trx.update(ref(uid,'wallets',wallet),{cachedBalance:increment(-delta),updatedAt:serverTimestamp()});trx.delete(t.ref);}trx.delete(rr);});await syncSnapshot(uid,opening?.data().date||'0000-01-01');
}
export async function attachClaimReceipt(uid:string,id:string,file:File){
  if(file.size>10*1024*1024)throw Error('Berkas maksimal 10 MB.');
  ...
  await updateDoc(ref(uid,'claims',id),{attachmentPath:path,updatedAt:serverTimestamp()});
}
...
export async function mergeCategory(uid:string,from:Category,to:Category){if(from.id===to.id||from.type!==to.type||Boolean(from.parentId)!==Boolean(to.parentId))throw Error('Pilih kategori tujuan dengan jenis dan tingkat yang sama.');
  ...
  for(let i=0;i<affected.length;i+=300){const batch=writeBatch(database());for(const item of affected.slice(i,i+300))batch.update(item.r,{[item.field]:to.id,updatedAt:serverTimestamp()});await batch.commit();}
