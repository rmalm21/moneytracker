...
export async function initProfile(user:User) {
  const r=userRef(user.uid); let snapshot=await getDocFromCache(r).catch(()=>null);
  if (!snapshot?.exists()) snapshot=await getDoc(r);
  ...
  // Last login is noted at most twice a day, not on every open.
  else { let last=0;try{last=Number(localStorage.getItem(loginKey(user.uid)))||0}catch{/* ignore */} if(Date.now()-last>12*36e5){await settle(updateDoc(r,{lastLoginAt:serverTimestamp()}));try{localStorage.setItem(loginKey(user.uid),String(Date.now()))}catch{/* ignore */}} }
}
...
export function subscribeData(uid:string,start:string,end:string,onPart:(key:Name,value:unknown[])=>void,onError:(e:Error)=>void,onSync?:(state:'syncing'|'synced'|'offline')=>void,asOf=new Date(),salaryDay=24):Unsubscribe {
  ...
  all.push(watchSync(status=>{current=status.current;if(status.failed&&status.failed!==failed){failed=status.failed;onError(Object.assign(new Error('Sinkronisasi data gagal.'),{code:status.failed}))}report();}));
  window.addEventListener('online',report);window.addEventListener('offline',report);all.push(()=>{window.removeEventListener('online',report);window.removeEventListener('offline',report)});
  for(const name of names) {
    ...
    all.push(onSnapshot(coll(uid,name),local,s=>{
      ...
      if(earliest<baseline||latest>baselineEnd)customStop=onSnapshot(query(coll(uid,'transactions'),where('date','>=',earliest),where('date','<',latest)),local,history=>{custom=history.docs.map(d=>hydrate<LedgerTx>(d));emit();},onError);
    },onError));
  }
  all.push(onSnapshot(query(coll(uid,'transactions'),orderBy('date','desc'),limit(60)),{...local,includeMetadataChanges:true},s=>{recent=s.docs.map(d=>hydrate<LedgerTx>(d));emit();pending=s.metadata.hasPendingWrites;report();},onError));
  all.push(onSnapshot(query(coll(uid,'transactions'),where('date','>=',start),where('date','<',end)),local,s=>{cycle=s.docs.map(d=>hydrate<LedgerTx>(d));emit();},onError));
  if(month.start!==start||month.end!==end)all.push(onSnapshot(query(coll(uid,'transactions'),where('date','>=',month.start),where('date','<',month.end)),local,s=>{calendar=s.docs.map(d=>hydrate<LedgerTx>(d));emit();},onError));
...
export async function archiveOrDelete(uid:string,name:Name,id:string,data:Data) {
  if(name==='wallets') {const fields=['walletId','destinationWalletId'] as const;const used=(await Promise.all(fields.map(field=>readLocalFirst(uid,['transactions'],query(coll(uid,'transactions'),where(field,'==',id),limit(1)))))).some(snapshot=>!snapshot.empty)||data.funds.some(f=>f.linkedWalletId===id)||data.recurring.some(r=>r.walletId===id)||data.plannedTransactions.some(p=>p.walletId===id)||data.claims.some(c=>c.sourceWalletId===id)||data.receivables.some(c=>c.sourceWalletId===id);if(used){await settle(updateDoc(ref(uid,name,id),{isArchived:true,updatedAt:serverTimestamp()}));return 'archived';}}
  if(name==='categories') {const fieldHits=await Promise.all((['categoryId','subcategoryId','transferFeeCategoryId'] as const).map(field=>readLocalFirst(uid,['transactions'],query(coll(uid,'transactions'),where(field,'==',id),limit(1)))));const nested=(await loadAllTransactions(uid)).some(t=>t.splits?.some(line=>line.categoryId===id||line.subcategoryId===id));const used=fieldHits.some(snapshot=>!snapshot.empty)||nested||data.budgets.some(b=>b.categoryId===id||b.subcategoryId===id)||data.categories.some(c=>c.parentId===id)||data.recurring.some(r=>r.categoryId===id)||data.plannedTransactions.some(p=>p.categoryId===id||p.subcategoryId===id);if(used){await settle(updateDoc(ref(uid,name,id),{isArchived:true,updatedAt:serverTimestamp()}));return 'archived';}}
  if(name==='budgets'||name==='funds'||name==='recurring'||name==='wallets'||name==='categories') {if(name==='funds' && !(await readLocalFirst(uid,['transactions'],query(coll(uid,'transactions'),where('fundId','==',id),limit(1)))).empty){await settle(updateDoc(ref(uid,name,id),{isArchived:true,updatedAt:serverTimestamp()}));return 'archived';}const b=writeBatch(database());b.delete(ref(uid,name,id));await settle(b.commit());await noteDeleted(uid,{[name]:[id]});return 'deleted';}
...
export async function saveDisplayOrder(uid:string,name:'categories'|'wallets'|'budgets'|'wishlist',ids:string[]){
 ...
 const field=name==='wallets'?'displayOrder':'sortOrder',known=new Map<string,unknown>();
 // Only the items whose position changed are written.
 try{(await getDocsFromCache(coll(uid,name))).docs.forEach(row=>known.set(row.id,row.get(field)))}catch{/* write them all */}
 const moved=ids.map((id,index)=>[id,index] as const).filter(([id,index])=>known.get(id)!==index);if(!moved.length)return;
 const batch=writeBatch(database());moved.forEach(([id,index])=>batch.update(ref(uid,name,id),{[field]:index,updatedAt:serverTimestamp()}));await settle(batch.commit());
}
...
export async function createDebt(uid:string,item:Omit<Debt,'id'|'outstandingAmount'|'status'|'createdAt'|'updatedAt'>,creditWallet?:string,date?:string) {if(!validAmount(item.originalAmount))throw Error('Nominal tidak valid.');const r=doc(coll(uid,'debts'));const batch=writeBatch(database());batch.set(r,{...item,outstandingAmount:item.originalAmount,status:'open',createdAt:serverTimestamp(),updatedAt:serverTimestamp()});let borrowedOn='';if(creditWallet){const t=doc(coll(uid,'transactions'));const {id:_d,...debtTx}=newTx({type:'borrowing',amount:item.originalAmount,walletId:creditWallet,debtId:r.id,description:item.name,date});batch.set(t,{...debtTx,createdAt:serverTimestamp(),updatedAt:serverTimestamp()});borrowedOn=debtTx.date;batch.update(ref(uid,'wallets',creditWallet),{cachedBalance:increment(item.originalAmount),updatedAt:serverTimestamp()});}await settle(batch.commit());if(borrowedOn)await syncSnapshot(uid,borrowedOn);return r.id;}
...
export async function settleBudgets(uid:string,budgets:Budget[],salaryDay:number,now=new Date()) {
  for(const budget of budgets.filter(b=>b.rolloverEnabled)) {
    ...
    const categories=(await readLocalFirst(uid,['categories'],coll(uid,'categories'))).docs.map(x=>hydrate<Category>(x));
    while(cursor<current.start&&steps++<(budget.cycleType==='weekly'?160:36)){const next=budgetWindow(budget,new Date(cursor+'T12:00:00'),salaryDay).end;if(next<=cursor)throw Error('Periode sisa anggaran tidak dapat dihitung.');const snap=await readLocalFirst(uid,['transactions'],query(coll(uid,'transactions'),where('date','>=',cursor),where('date','<',next)));const tx=snap.docs.map(x=>hydrate<LedgerTx>(x));carry+=budget.amount-budgetSpent(budget,tx,categories);cursor=next;}
...
export async function mergeCategory(uid:string,from:Category,to:Category){if(from.id===to.id||from.type!==to.type||Boolean(from.parentId)!==Boolean(to.parentId))throw Error('Pilih kategori tujuan dengan jenis dan tingkat yang sama.');
  ...
  // Budgets that cover several subcategories keep the list in subcategoryIds.
  const swap=(ids:string[]|undefined)=>ids?[...new Set(ids.map(id=>id===from.id?to.id:id))]:ids;
  const multi=await getDocs(query(coll(uid,'budgets'),where('subcategoryIds','array-contains',from.id)));for(let i=0;i<multi.docs.length;i+=300){const batch=writeBatch(database());for(const row of multi.docs.slice(i,i+300))batch.update(row.ref,{subcategoryIds:swap(row.data().subcategoryIds as string[]),updatedAt:serverTimestamp()});await settle(batch.commit());}
...
export async function deleteWish(uid:string,id:string){ const b=writeBatch(database()); b.delete(ref(uid,'wishlist',id)); await settle(b.commit()); await noteDeleted(uid,{wishlist:[id]}); }
