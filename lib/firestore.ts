...
export async function archiveOrDelete(uid:string,name:Name,id:string,data:Data) {
  ...
  if(name==='budgets'||name==='funds'||name==='recurring'||name==='wallets'||name==='categories') {if(name==='funds' && !(await getDocs(query(coll(uid,'transactions'),where('fundId','==',id),limit(1)))).empty){await updateDoc(ref(uid,name,id),{isArchived:true,updatedAt:serverTimestamp()});return 'archived';}const b=writeBatch(database());b.delete(ref(uid,name,id));await b.commit();return 'deleted';}
