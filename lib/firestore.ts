...
export async function createDebt(uid:string,item:Omit<Debt,'id'|'outstandingAmount'|'status'|'createdAt'|'updatedAt'>,creditWallet?:string) {if(!validAmount(item.originalAmount))throw Error('Nominal tidak valid.');const r=doc(coll(uid,'debts'));const batch=writeBatch(database());batch.set(r,{...item,outstandingAmount:item.originalAmount,status:'open',createdAt:serverTimestamp(),updatedAt:serverTimestamp()});if(creditWallet){const t=doc(coll(uid,'transactions'));const {id:_d,...debtTx}=newTx({type:'borrowing',amount:item.originalAmount,walletId:creditWallet,debtId:r.id,description:item.name});batch.set(t,{...debtTx,createdAt:serverTimestamp(),updatedAt:serverTimestamp()});batch.update(ref(uid,'wallets',creditWallet),{cachedBalance:increment(item.originalAmount),updatedAt:serverTimestamp()});}await settle(batch.commit());await syncSnapshot(uid,item.dueDate||'0000-01-01');return r.id;}
...
/** Move money between pockets of the same Tabungan wallet (or from/to "belum dialokasikan" when an id is null). No wallet transaction: the money never leaves the wallet. */
export async function movePocketMoney(uid:string,fromFundId:string|null,toFundId:string|null,amount:number){ if(!validAmount(amount))throw Error('Nominal harus lebih dari nol.'); if(fromFundId===toFundId)return; const b=writeBatch(database()); if(fromFundId)b.update(ref(uid,'funds',fromFundId),{currentAmount:increment(-amount),updatedAt:serverTimestamp()}); if(toFundId)b.update(ref(uid,'funds',toFundId),{currentAmount:increment(amount),updatedAt:serverTimestamp()}); await settle(b.commit()); }
