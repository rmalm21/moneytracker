...
export async function repairWalletCache(uid:string,id:string,expected:number){
  ...
  if(preview.cached!==expected)throw Error('Saldo berubah. Periksa ulang sebelum memperbaiki cache.');
  ...
  try{await refreshCycleSnapshots(uid,'0000-01-01');return {balance:preview.calculated,warning:''}}catch(error){console.error(error);return {balance:preview.calculated,warning:'Cache sudah diperbaiki, tetapi ringkasan siklus belum dapat diperbarui. Buka Riwayat Siklus → Perbarui Laporan.'}}
}
export async function reconcileWallet(uid:string,id:string,expectedCached:number,actual:number,notes:string,date?:string){
  ...
  const preview=await walletBalancePreview(uid,id);
  if(preview.cached!==preview.calculated)throw Error('Saldo cache berbeda dari ledger. Jalankan Hitung Ulang Saldo dahulu.');
...
function verifyBalances(data:Data,ledger:LedgerTx[]){for(const wallet of data.wallets){if(walletBalance(wallet,ledger)!==wallet.cachedBalance)throw Error(`Saldo cache ${wallet.name} berbeda dari transaksi. Buka Dompet → Hitung Ulang Saldo sebelum memperbarui laporan siklus.`)}}
