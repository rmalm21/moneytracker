...
export function analyzeFinances(input: AdvisorInput): Advice {
  ...
  // Savings outside the emergency kantong fill a short emergency fund first; only what is left is spare.
  const savingsToEmergency = pocketsEmergency ? Math.min(unallocatedSavings, emergencyShortfall) : 0;
  const shortfallLeft = emergencyShortfall - savingsToEmergency;
  const savingsExcess = pocketsEmergency ? Math.max(0, unallocatedSavings - savingsToEmergency + Math.max(0, emergencyCash - emergencyTarget) - (wishSaved - wishFromOp)) : Math.max(0, emergencyCash - emergencyTarget - (wishSaved - wishFromOp));
  ...
  const savingsCalc: CalcRow[] = pocketsEmergency ? [{ label: 'Tabungan di luar kantong', amount: unallocatedSavings }, ...(savingsToEmergency ? [{ op: '-' as const, label: 'Dipakai melengkapi dana darurat', amount: savingsToEmergency, note: `dana darurat masih kurang ${rp(emergencyShortfall)}` }] : []), { op: '+', label: 'Kelebihan kantong dana darurat', amount: Math.max(0, emergencyCash - emergencyTarget), note: `di atas target ${targetText}` }, ...(wishSaved - wishFromOp > 0 ? [{ op: '-' as const, label: 'Disisihkan untuk wish list', amount: wishSaved - wishFromOp }] : []), { op: '=', label: 'Kelebihan tabungan', amount: savingsExcess }] : [...emergencyCalc.slice(targetRows.length), { op: '-', label: `Target dana darurat (${targetText})`, amount: emergencyTarget, note: needNote }, ...(wishSaved - wishFromOp > 0 ? [{ op: '-' as const, label: 'Disisihkan untuk wish list', amount: wishSaved - wishFromOp }] : []), { op: '=', label: 'Kelebihan tabungan', amount: savingsExcess }];
  ...
  const afterEmergency = Math.max(0, idleTotal - shortfallLeft);
  ...
  const idleCalc: CalcRow[] = [{ label: 'Menganggur di dompet harian', amount: idleOperational }, { op: '+', label: 'Kelebihan tabungan', amount: savingsExcess }, { op: '=', label: 'Total uang menganggur', amount: idleTotal }, ...(shortfallLeft ? [{ op: '-' as const, label: 'Dipakai melengkapi dana darurat', amount: Math.min(idleTotal, shortfallLeft) }] : []), ...(debtFirst ? [{ op: '-' as const, label: 'Dipakai melunasi utang berbunga ≥ 8%', amount: debtFirst }] : []), ...(runwayShort ? [{ op: '-' as const, label: 'Ditahan: uang tersedia kurang sampai gajian', amount: afterEmergency - debtFirst }] : []), { op: '=', label: 'Siap diinvestasikan', amount: investable }];
